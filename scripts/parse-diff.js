  ...
  for (let i = 0; i < diffLines.length; i++) {
    ...
    // Detect hunk header (e.g., @@ -0,0 +1,1495 @@)
      ...
      if (match) {
        ...
        while (
          j < diffLines.length &&
          !diffLines[j].startsWith("@@") &&
          !diffLines[j].startsWith("diff --git")
        ) {
        ...
        changes[currentFile].push({
          ...
          context:
            context.length > 0 ? context : [{ start: "No content", end: "" }],
        });
        console.log(
          `Detected change in ${currentFile}: lines ${startLine}-${endLine}, context:`,
          context
        );
      } else {
